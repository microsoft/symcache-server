...
will support new minor versions within the same major version, so the server can always just run the lates transcoder
within each major version. This sample only includes code to run a single transcoder version at one time, and it ignores
files in its cache directory from any other version.
